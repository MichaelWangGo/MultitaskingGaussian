...
class SceneInfo(NamedTuple):
    ...
    # point_cloud_tool : BasicPointCloud
    train_cameras: list
    test_cameras: list
    video_cameras: list
...
def storePly(path, xyz, rgb):
    ...
    elements = np.empty(xyz.shape[0], dtype=dtype) #创建了一个长度为3000的数组，每个元素都包含空间坐标、法线向量和颜色信息，以结构化数据的形式存储
...
def readEndoNeRFInfo(datadir, mode):
    ...
    for i in range(len(xyz)):
        normals[i] = np.random.random((xyz[i].shape[0], 3))
        pcd.append(BasicPointCloud(points=xyz[i], colors=rgb[i], normals=normals[i]))
        ply_path_interim = os.path.join(datadir, f"my_points3d_{i}.ply")
        ply_path.append(os.path.join(datadir, f"my_points3d_{i}.ply"))
        storePly(ply_path_interim, xyz[i], rgb[i]*255)

    # ply_path = os.path.join(datadir, "points3d.ply")

    # ply_path = os.path.join(datadir, "my_points3d.ply")
    # storePly(ply_path, xyz, rgb*255)

    try:
        # for i in range(len(pcd)):
        #     pcd[i] = fetchPly(ply_path[i])
        
        pcds = [fetchPly(ply_path[i]) for i in range(len(ply_path))]

        points = pcds[0].points
        colors = pcds[0].colors
        normals = pcds[0].normals
        for i in range(1, len(pcds)):
            points = np.concatenate((points, pcds[i].points), axis=0)
            colors = np.concatenate((colors, pcds[i].colors), axis=0)
            normals = np.concatenate((normals, pcds[i].normals), axis=0)
        # import ipdb; ipdb.set_trace()
