...
class EndoNeRF_Dataset(object):
    ...
    def load_meta(self):
        ...
        poses_arr = np.load(os.path.join(self.root_dir, "poses_bounds.npy")) #(63,17)
    ...
    def get_init_pts(self, sampling='random'):
        if self.mode == 'binocular':
            final_pts, final_colors, final_normals = [], [], []
            for idx in self.train_idxs:
                pts_total, colors_total, final_pts_total, final_colors_total = [], [], [], []
                ...
                color, depth, tool_info = self.get_color_depth_mask(idx, num_class=1, mode=self.mode)
                scene_pts, scene_colors, tool_pts_color = self.get_pts_cam(depth, tool_info, color, disable_mask=False)
                pts_total.append(scene_pts)
                colors_total.append(scene_colors)
                for i in range(len(tool_pts_color)):
                    
                    pts_total.append(tool_pts_color[i]["tool_pts"])
                    colors_total.append(tool_pts_color[i]["tool_color"])
                for i in range(len(pts_total)):
                    pts_total[i] = self.get_pts_wld(pts_total[i], self.image_poses[idx])

                    num_pts = pts_total[i].shape[0]
                    if sampling == 'fps':
                    ...
                    pts_sel, colors_sel = pts_total[i][sel_idxs], colors_total[i][sel_idxs]
                    
                    final_pts_total.append(pts_sel)
                    final_colors_total.append(colors_sel)
            for i in range(len(final_pts_total)):
                final_pts_total.append(np.concatenate(final_pts_total[i]))
                final_colors_total.append(np.concatenate(final_colors_total[i]))
                sel_idxs = np.random.choice(final_pts_total[i].shape[0], 30_000, replace=True)
                final_pts.append(final_pts_total[i][sel_idxs])
                final_colors.append(final_colors_total[i][sel_idxs])
                # final_pts[i], final_colors[i] = final_pts_total[i][sel_idxs], final_colors_total[i][sel_idxs]
                final_normals.append(np.zeros((final_pts[i].shape[0], 3)))
        ...
        # import ipdb; ipdb.set_trace()
        return final_pts, final_colors, final_normals
    ...
    def get_color_depth_mask(self, idx, num_class, mode):
        tool = {}
        if mode == 'binocular':
        ...
        if num_class > 0:
            for i in range(num_class):
                tool[i] = {}
                tool[i]["tool"] = np.array(Image.open(self.masks_paths[idx]))/255.0
                tool[i]["color"] = np.array(Image.open(self.image_paths[idx]))/255.0
                tool[i]["mask"] = 1 - np.array(Image.open(self.masks_paths[idx]))/255.0
        # mask = 1 - np.array(Image.open(self.masks_paths[idx]))/255.0
        color = np.array(Image.open(self.image_paths[idx]))/255.0
        return color, depth, tool
    ...
    def get_pts_cam(self, depth, mask, color, disable_mask=False):
        ...
        if not disable_mask:
            ...
            for i in range(len(mask)):
                tool_mask.append(mask[i]["mask"])
                tool_color.append(mask[i]["color"])
                tool.append(mask[i]["tool"])

                tool_mask[i] = tool_mask[i].reshape(-1).astype(bool)
                scene_pts = pts_cam[tool_mask[i], :]
                scene_color = color[tool_mask[i], :]

                tool[i] = tool[i].reshape(-1).astype(bool)
                tool_pts = pts_cam[tool[i], :]
                tool_color = color[tool[i], :]

                tool_pts_color[i] = {"tool_pts": tool_pts, "tool_color": tool_color}

            # tool_mask = tool_mask.reshape(-1).astype(bool)
            # pts_valid = pts_cam[tool_mask, :]
            # color_valid = color[tool_mask, :]
        ...
        return scene_pts, scene_color, tool_pts_color
